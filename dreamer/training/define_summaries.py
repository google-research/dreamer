...
def define_summaries(graph, config, cleanups):
  ...
  heads.pop('cpc', None)  # Not applied to RNN states.

  with tf.variable_scope('general'):
    summaries += summary.data_summaries(graph.data, config.postprocess_fn)
    summaries += summary.dataset_summaries(config.train_dir, 'train_dataset')
    summaries += summary.dataset_summaries(config.test_dir, 'test_dataset')
  ...
  with tf.variable_scope('closedloop'):
    ...
    #   summaries += summary.dist_summaries(prior_dists, graph.data)
  ...
  with tf.variable_scope('openloop'):
    ...
    if 'cpc' in graph.heads:
      state_dists['cpc'] = graph.heads.cpc(graph.embedded)
    summaries += summary.dist_summaries(state_dists, graph.data)
    summaries += summary.image_summaries(
  ...
  with tf.variable_scope('simulation'):
    ...
    for name, params in config.test_collects.items():
      ...
      sim_summary, sim_return = tf.cond(
          ...
          lambda: utility.simulate(
              graph.metrics, config, params, graph, cleanups,
              gif_summary=True, name=name),
          lambda: ('', 0.0),
