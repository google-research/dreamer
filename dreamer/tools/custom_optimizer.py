...
class CustomOptimizer(object):
  ...
  def __init__(
    ...
    self._debug = debug
    self._name = name
    self._optimizer = optimizer_cls(learning_rate=learning_rate, name=name)
  ...
  def minimize(self, loss):
    ...
    if hasattr(self._optimizer, 'compute_gradients'):  # TF 1.0
      gradients, variables = zip(*self._optimizer.compute_gradients(
          loss, self._variables, colocate_gradients_with_ops=True))
    else:  # Keras
      gradients = self._optimizer.get_gradients(loss, self._variables)
